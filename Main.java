...
public class Main {
    public static void main(String[] args) {
        ...
        do { howManyPencils = scanner.nextLine(); } while (!howManyPencilsValidate(howManyPencils));

        System.out.println("Who will be the first (" + participants[0] + ", " + participants[1] + "):");
        do { whoWillBe = scanner.nextLine(); } while (!participantValidate(whoWillBe,participants));

        if (whoWillBe.equals(participants[0])) {user = 0; boot = 1;}
        if (whoWillBe.equals(participants[1])) {user = 1; boot = 1;}
        ...
        do {
            ...
            else do { takenPencils = scanner.nextLine();
